...
export default function ReorderReportModal({ isOpen, onClose }) {
    ...
    const fetchReportData = async () => {
        try {
            ...
            const response = await axios.get('/api/seller/reports/reorder/data', {
                headers: {
            ...
            console.error('Error fetching reorder report data:', error);
            setReportData([]);
        } finally {
            setLoading(false);
        }
    ...
    const handleDownloadPdf = async () => {
        try {
            setDownloading(true);
            const token = localStorage.getItem('token');
            const response = await axios.get('/api/seller/reports/reorder', {
                headers: {
                    'Authorization': `Bearer ${token}`
                },
                responseType: 'blob'
            ...
            link.href = url;
            link.setAttribute('download', `Laporan_Reorder_${new Date().toISOString().split('T')[0]}.pdf`);
        ...
        <div className={styles.modalOverlay} onClick={onClose}>
            <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
                <div className={styles.modalHeader}>
                    <h2 className={styles.modalTitle}>Laporan Produk Perlu Restock</h2>
                                        ...
                                        {reportData.length === 0 ? (
                                            ...
                                            reportData.map((item, index) => (
                                                <tr key={item.id || index}>
                                                    ...
                                                    <td className={styles.textCenter}>{item.stok}</td>
                                                </tr>
                                            ))
                                        )}
                                    </tbody>
