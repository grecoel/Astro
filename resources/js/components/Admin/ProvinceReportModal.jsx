...
export default function ProvinceReportModal({ isOpen, onClose }) {
    ...
    const fetchReportData = async () => {
        try {
            ...
            const response = await axios.get('/api/admin/reports/sellers/province/data', {
                headers: {
    ...
    const handleDownloadPdf = async () => {
        try {
            setDownloading(true);
            const token = localStorage.getItem('token');
            const response = await axios.get('/api/admin/reports/sellers/province/pdf', {
                headers: {
                    'Authorization': `Bearer ${token}`
                },
                responseType: 'blob'
            ...
            link.href = url;
            link.setAttribute('download', `Laporan_Penjual_Per_Provinsi_${new Date().toISOString().split('T')[0]}.pdf`);
        ...
        <div className={styles.modalOverlay} onClick={onClose}>
            <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
                <div className={styles.modalHeader}>
                    <h2 className={styles.modalTitle}>Laporan Daftar Toko Berdasarkan Lokasi Provinsi</h2>
                            ...
                            <div className={styles.reportHeader}>
                                ...
                                <div className={styles.reportMeta}>
                                    ...
                                    <div className={styles.metaItem}>
                                        ...
                                        <span className={styles.metaValue}>{reportData.length} Toko</span>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <table className={styles.reportTable}>
                                        ...
                                        <tbody>
                                            {reportData.map((seller) => (
                                                <tr key={seller.no}>
                                                    <td className={styles.textCenter}>{seller.no}</td>
